#!/usr/bin/env python3
"""Profile SPMM share in full ClusterLoader + GCN training.

This standalone script profiles complete ClusterGCN training epochs on the
default five datasets:

  ogbn_arxiv, ogbn_products, flickr, yelp, reddit
...
DEFAULT_DATASETS = ("ogbn_arxiv", "ogbn_products", "flickr", "yelp", "reddit")
...
DEFAULT_CLUSTER_PARTS = {
    "ogbn_arxiv": 1500,
    "ogbn_products": 5000,
    "flickr": 512,
    "yelp": 3000,
    "amazon_products": 8000,
    "reddit": 1500,
}
...
class ClusterGCN(nn.Module):
    """Fixed GCN model for ClusterLoader mini-batch training."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Profile SPMM CUDA-time share for full ClusterGCN training"
    )
    ...
    parser.add_argument("--output-dir", default="results/cluster_gcn_profile")
    parser.add_argument(
        "--directed-ogbn-arxiv",
        action="store_true",
...
def loader_args(args: argparse.Namespace, dataset: str) -> SimpleNamespace:
    cluster_parts = args.cluster_parts
    if cluster_parts <= 0:
        cluster_parts = DEFAULT_CLUSTER_PARTS.get(dataset, 1500)
...
def make_loader(bundle: DatasetBundle, args: SimpleNamespace):
...
def train_step(model, optimizer, batch, device: torch.device) -> bool:
    ...
    loss = classification_loss(out[mask], batch.y[mask])
    loss.backward()
    optimizer.step()
    return True
...
def summarize_profile(events: Sequence[object], steps: int) -> dict:
    if steps <= 0:
        raise RuntimeError("no non-empty ClusterGCN training steps were profiled")
...
def run_dataset(dataset: str, args: argparse.Namespace, device: torch.device) -> dict:
    ...
    largs = loader_args(args, dataset)
    loader = make_loader(bundle, largs)
    model = ClusterGCN(
...
def main() -> int:
    ...
    summary_csv = Path(args.output_dir) / "cluster_gcn_spmm_share_summary.csv"

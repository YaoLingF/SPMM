#!/usr/bin/env python3
"""Train ClusterLoader + OurGCNConv for full-epoch comparisons."""
...
from our_gnn import OurGCNConv, build_gcn_csr_pack, preload_our_spmm_extension  # noqa: E402
from profile_cluster_gcn_spmm import DEFAULT_CLUSTER_PARTS  # noqa: E402


class OurClusterGCN(nn.Module):
    """Fixed GCN stack backed by OurGCNConv."""
    ...
    def __init__(
        ...
        if num_layers == 1:
            self.convs.append(OurGCNConv(in_channels, out_channels))
        else:
            self.convs.append(OurGCNConv(in_channels, hidden_channels))
            for _ in range(num_layers - 2):
                self.convs.append(OurGCNConv(hidden_channels, hidden_channels))
            self.convs.append(OurGCNConv(hidden_channels, out_channels))
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Full OurGCN ClusterGCN training")
    parser.add_argument(
        "--dataset",
    ...
    parser.add_argument("--output-dir", default="results/ourgcn_sampled_train")
    parser.add_argument(
        "--directed-ogbn-arxiv",
        action="store_true",
...
def train_batch(model, optimizer, batch, args: argparse.Namespace, device: torch.device):
    ...
    csr_pack = build_gcn_csr_pack(
        batch.edge_index, int(batch.num_nodes), dtype=batch.x.dtype
    )
...
def main() -> int:
    ...
    print(
        f"dataset={args.dataset} loader=cluster variant=our "
    ...
    print(
        f"OurClusterGCN layers={args.num_layers} hidden={args.hidden_channels} "
    ...
    csv_path = output_dir / f"{args.dataset}_cluster_our_train.csv"

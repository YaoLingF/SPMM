                                 ...
                                 int *num_warps_per_row, int *row_idx_for_each_warp, int *warpid_within_row)
{
    ...
    if (target_row == -1)
        return;
    int row_nnz = row_offset[target_row + 1] - row_offset[target_row];
    ...
    if (no_warps_within_row <= num_warps && row_nnz <= 64)
    {
...
void our_spmm_balanced(CSR *A_csr, half *B, float *C, int N, int n_iter, double *elapsed)
{
    ...
    cudaDeviceSynchronize();
    // num_block = M;
    VERBOSE_PRINT("num_block: %d blockdim: %d shm: %fKB\n", num_block, blockdim, (float)shm_size / 1024.0f);
    for (int i = 0; i < 5; i++)
    {
